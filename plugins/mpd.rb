  ...
  def init(init)
    ...
    if ( @@bot[:messages] ) && ( @@bot[:mpd].nil? ) then
      ...
      @@bot[:mpd].on :updating_db do |jobid|
        channelmessage( "I am running a database update just now ... new songs :)<br>My job id is: #{jobid}.") if (@@bot["channel_notify"] & 0x02) != 0
      end
      ...
      @@bot[:mpd].on :random do |random|
        ...
        channelmessage( "Random mode is now: #{random}.") if (@@bot["channel_notify"] & 0x04) != 0
      end

      @@bot[:mpd].on :state  do |state|
        if @@bot["channel_notify"] & 0x80 != 0 then
      ...
      @@bot[:mpd].on :single do |single|
        ...
        channelmessage( "Single mode is now: #{single}.") if (@@bot["channel_notify"] & 0x08) != 0
      end
      ...
      @@bot[:mpd].on :consume do |consume|
        ...
        channelmessage( "Consume mode is now: #{consume}.") if (@@bot["channel_notify"] & 0x10) != 0
      end
      ...
      @@bot[:mpd].on :xfade do |xfade|
        if xfade.to_i == 0
          ...
          channelmessage( "Crossfade time (in seconds) is now: #{xfade}.") if (@@bot["channel_notify"] & 0x20) != 0
        end
      end
      ...
      @@bot[:mpd].on :repeat do |repeat|
        ...
        channelmessage( "Repeat mode is now: #{repeat}.") if (@@bot["channel_notify"] & 0x40) != 0
      end
            ...
            if ( lastcurrent.title != current.title ) || ( init == true )
              ...
              if @@bot["main"]["display"]["comment"] == true && @@bot[:set_comment_available] == true
                ...
                if current.artist.nil? && current.title.nil? && current.album.nil?
                  channelmessage( "#{current.file}") if ( @@bot["channel_notify"] && 0x80 ) == true
                else
                  channelmessage( "#{current.artist} - #{current.title} (#{current.album})") if (@@bot["channel_notify"] && 0x80) != 0
                end
              end
            lastcurrent = current
